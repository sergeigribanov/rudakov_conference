...
\begin{frame}
  ...
  \begin{minipage}[t]{0.59\linewidth}
    ...
    \end{figure}
    \begin{itemize}
    \item{\scriptsizeОбласть сканирования $0.3\text{ ГэВ} < \sqrt{s} < 2.0\text{ ГэВ}$.}
    \item{\scriptsizeЭнергия измеряется методом обратного комптоновского рассеяния с точностью
        $50\text{ КэВ}$.}
    \item{\scriptsizeВпервые использована методика круглых пучков.}
    \item{\scriptsizeДостигнута светимость
        $6\times{10^{31}}\text{ см}^{-2}\text{с}^{-1}$~(проектная~$\sim{10^{32}}\text{ см}^{-2}\text{c}^{-1}$).}
    \item{\scriptsizeДва детектора КМД-3 и СНД.}
    \end{itemize}
  \end{minipage}
...
\begin{frame}
  ...
  \begin{frame}
    ...
    \item{\scriptsizeЧисло событий определяемое из фита $m_{\gamma\gamma}$ используется для получения
        сечения.}
...
\begin{frame}
  ...
  \begin{itemize}
    \item{\scriptsizeДля уточнения модели фона был проведен амплитудный анализ процесса
        $e^+e^-\to{4\pi}$. }
      \item{\scriptsizeПолучен предварительный результат, достаточный для уточнения
    ...
    \item{\scriptsize Проводились амплитудные фит для различных конечных состояний:}
      ...
      \end{itemize}
    \item{\scriptsizeПромежуточные состояния~(список приблизительный, использовались
